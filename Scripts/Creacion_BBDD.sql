...
(
  ...
  Codigo_postal smallint, -- con esto se calculará el envío?
  Telefono varchar(20),
  Email varchar(30),
  Fecha_Baja date, 
...
INSERT INTO cli_clientes (Nombre,Apellido,Direccion,Localidad,Provincia,Codigo_postal,Telefono,Email) 
VALUES('Dario','Rick','Eva Peron 710','Los Polvorines','Buenos Aires',1613,'1566060529','dario_rick@outlook.com');
...
insert into env_estados_vehiculos (nombreEstado) values ("en reparación");
...
CREATE TABLE ot_costos_variables
(
 IdCostoV serial,
 Tipo_Costo varchar(30),
 Precio integer not null,
 PRIMARY KEY (IdCostoV)
);

INSERT INTO ot_costos_variables(Tipo_Costo,Precio) VALUES('COSTO_HORA_MANO_OBRA',300);
INSERT INTO ot_costos_variables(Tipo_Costo,Precio) VALUES('COSTO_FIJO_ENVIO',100);
...
INSERT INTO ot_estados(estado) VALUES('En reparación'); -- 5
...
CREATE TABLE ot_ordenes_trabajo (
  ...
  estado_orden bigint unsigned not null,
    ...
    FOREIGN KEY (idFleteroAsoc) REFERENCES env_fleteros (idFletero),
...
CREATE TABLE ot_items ( -- Cada fila representa un item del presupuesto
    idOTItems serial,
    idOT bigint unsigned not null, -- ot asociada
    IdCostoV bigint unsigned, -- costo de mano de obra o envío
    horasTrabajo integer,
    idProdPieza bigint unsigned,
    idProdStock bigint unsigned, -- item particular del presupuesto. Irá nulo hasta que se repare
    precio decimal(10,2),
    PRIMARY KEY (idOTItems),
    FOREIGN KEY (idOT) REFERENCES ot_ordenes_trabajo (idOT),
  FOREIGN KEY (IdCostoV) REFERENCES ot_costos_variables (IdCostoV),
  FOREIGN KEY (idProdPieza) REFERENCES prod_piezas (idProdPieza),
...
INSERT INTO ot_ordenes_trabajo(idCliente,idElectro,descripcion,idUsuarioAlta,esDelivery,estado_orden) 
VALUES(1,1,'OT ejemplo estado ingresada',1,false,1);

INSERT INTO ot_estados_historico(idOT,estado_orden,Fecha_Alta,Fecha_Baja)
VALUES(1,1,NOW(),NULL);

INSERT INTO ot_ordenes_trabajo(idCliente,idElectro,descripcion,idUsuarioAlta,esDelivery,estado_orden) 
VALUES(1,1,'OT ejemplo estado presupuestada',1,false,2);
...
INSERT INTO ot_items (idOT,idProdStock,precio) 
VALUES(2,1,100); -- Insert de un costo fijo (alguna pieza). Una vez que se repare, la pieza debe pasar a estado VENDIDA

INSERT INTO ot_items (idOT,IdCostoV,precio) 
VALUES(2,1,300); -- Insert de un costo variable (mano de obra por ejemplo)
...
(
  ...
  idOTItems bigint unsigned, -- FK con las órdenes de trabajo
  idSolcPiezas bigint unsigned, -- FK con las solicitudes de compra
  Fecha date not null,
  PRIMARY KEY (idOperacion),
  FOREIGN KEY (idOTItems) REFERENCES ot_items (idOTItems),
...
BEGIN
  ...
  ELSE
    ...
    THEN
      INSERT INTO ot_items (idOT,idProdPieza,precio)
      VALUES (idOT,idProdPieza,precio);
    END IF;
    
    IF (IdCostoV IS NOT NULL)
        THEN
      INSERT INTO ot_items(idOT,IdCostoV,horasTrabajo,precio)
      VALUES (idOT,IdCostoV,horasTrabajo,precio);
    END IF;
  END IF;
...
BEGIN
  ...
  DECLARE CONTINUE HANDLER FOR SQLEXCEPTION 
    SET retCode = 1, descErr = 'presupuestar() - Error General';
    
  SET retCode = 0;
    SET descErr = 'presupuestar() - OK';
   
   -- Llamar a cambiarvencimiento y cambiarEstado
  CALL cambiarVencimientoPresup(id,vencimiento,@retCode,@descErr);
  CALL cambiarEstadoOT(id,2,@retCode,@descErr); 
...
BEGIN
  ...
  THEN
	...
	END IF;
  	...
  	IF (vEstadoActual = 3 and idEstadoPosible = 5) -- Si pasa de Aprobada a En reparación
	THEN
      SET vEsCambioValido = 1;
	END IF;
    
	IF (vEstadoActual = 5 and idEstadoPosible = 7) -- Si pasa de En reparación a Reparada
	THEN
...
COMMENT 'Reporte de "Envíos del día"'
BEGIN
...
IF vidProdEstado = 2 -- Si se vendió una pieza
THEN
